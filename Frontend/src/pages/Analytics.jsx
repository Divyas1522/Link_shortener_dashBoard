...
const Analytics = () => {
  ...
  return (
    <div className="grid sm:grid-col-2  p-4 max-h-screen bg-[#0f111a] text-white">
      <h1 className="text-3xl font-bold text-blue-300 mt-5 mb-7">Analytics Dashboard</h1>

      {/* Clicks Over Time */}
      <div className="bg-[#1c1f2a] p-2 mb-6 rounded-xl shadow">
        <h2 className="text-xl font-semibold mb-3 text-blue-300">Clicks Over Time</h2>
        <ResponsiveContainer width="100%" height={250}>
          <PieChart>
          <Pie
          data={analytics.clicksOverTime.map(item => ({
            name: item.date, value: item.clicks
          }))}
            cx="50%"
            cy="50%"
            outerRadius={ 80}
            ...
            label 
          >
            {analytics.clicksOverTime.map((entry, index) => (
              <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
            ))}
          </Pie>
          <Tooltip />
          <Bar dataKey="clicks" fill="#00bcd4" />
          <Legend />
        </PieChart>
      </ResponsiveContainer>
    </div>
